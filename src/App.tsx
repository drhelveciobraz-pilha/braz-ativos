...
// O ngrok grátis intercepta requisições de navegador com uma página HTML de
// aviso. Sem este header, todo fetch cross-origin (Vercel -> ngrok) recebe
// esse HTML no lugar do JSON, r.json() estoura, e o app fica vazio/mudo.
// Este header manda o ngrok pular o aviso e proxyar direto pro FastAPI.
...
function useVarredura() {
  ...
  const consultarStatus = useCallback(async () => {
    try {
      const r = await fetch(`${API_BASE}/admin/status`, { headers: H });
  ...
  const rodarAgora = useCallback(async () => {
    try {
      const r = await fetch(`${API_BASE}/admin/rodar_agora`, { headers: H });
...
function useFetch(caminho, versao) {
  ...
  const buscar = useCallback(async () => {
    setCarregando(true);
    try {
      const r = await fetch(`${API_BASE}${caminho}`, { headers: H });
